...
use {io, Event, Evented, Ready, Registration, Poll, PollOpt, Token};
use magenta;
use iovec::IoVec;
use std::fmt;
...
impl Selector {
                  ...
                  timeout: Option<Duration>) -> io::Result<bool>
    {
        ...
        let packet = self.port.wait(deadline).map_err(status_to_io_err)?;
        let key = packet.key();
        ...
        let observed_signals = match packet.contents() {
            ...
            magenta::PacketContents::User(_user_packet) => {
                // User packets should only be generated by awakeners
                assert_eq!(key, awakener.0 as u64,
                    "user packet sent by wrong awakener: expected key {}, found {}",
                    key,
                    awakener.0 as u64
                );
                return Ok(true);
    ...
    pub fn register<H: magenta::HandleBase>(&self,
                                            ...
                                            options: magenta::WaitAsyncOpts) -> io::Result<()>
    {
        handle.wait_async(&self.port, token.0 as u64, signals, options)
