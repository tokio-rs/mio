...
impl Selector {
    ...
    pub fn register(&mut self, io: &IoDesc, token: usize, isizeerests: Interest, opts: PollOpt) -> MioResult<()> {
        debug!("registering; token={}; isizeerests={}", token, isizeerests);

        try!(self.ev_register(io, token, EVFILT_READ, isizeerests.contains(event::READABLE), opts));
        try!(self.ev_register(io, token, EVFILT_WRITE, isizeerests.contains(event::WRITABLE), opts));

        Ok(())
    }

    pub fn reregister(&mut self, io: &IoDesc, token: usize, isizeerests: Interest, opts: PollOpt) -> MioResult<()> {
        // Just need to call register here since EV_ADD is a mod if already
        // registered
        self.register(io, token, isizeerests, opts)
    }
