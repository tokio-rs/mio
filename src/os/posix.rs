...
pub fn join_multicast_group(io: &IoDesc, addr: &IpAddr, isizeerface: &Option<IpAddr>) -> MioResult<()> {
    let grp_req = try!(make_ip_mreq(addr, isizeerface));

    nix::setsockopt(io.fd, nix::IPPROTO_IP, nix::IP_ADD_MEMBERSHIP, &grp_req)
        .map_err(MioError::from_sys_error)
}

pub fn leave_multicast_group(io: &IoDesc, addr: &IpAddr, isizeerface: &Option<IpAddr>) -> MioResult<()> {
    let grp_req = try!(make_ip_mreq(addr, isizeerface));
