...
use event_imp::{Event, Interests, PollOpt};
...
impl<H: Handler> EventLoop<H> {
    ...
    fn configured(config: Config) -> io::Result<EventLoop<H>> {
        ...
        poll.register(&rx, NOTIFY, Interests::readable(), PollOpt::edge() | PollOpt::oneshot())?;
        poll.register(&timer, TIMER, Interests::readable(), PollOpt::edge())?;
    ...
    pub fn register<E: ?Sized>(&mut self, io: &E, token: Token, interest: Interests, opt: PollOpt) -> io::Result<()>
        where E: Evented
    {
    ...
    pub fn reregister<E: ?Sized>(&mut self, io: &E, token: Token, interest: Interests, opt: PollOpt) -> io::Result<()>
        where E: Evented
    {
    ...
    fn notify(&mut self, handler: &mut H) {
        ...
        // Re-register
        let _ = self.poll.reregister(&self.notify_rx, NOTIFY, Interests::readable(), PollOpt::edge() | PollOpt::oneshot());
    }
