...
use {convert, io, Ready, Interests, Poll, PollOpt, Registration, SetReadiness, Token};
use event::Evented;
...
impl<T> Evented for Timer<T> {
    fn register(&self, poll: &Poll, token: Token, interest: Interests, opts: PollOpt) -> io::Result<()> {
