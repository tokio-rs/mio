...
impl EchoConn {
  ...
  fn writable(&mut self, event_loop: &mut TestEventLoop) -> MioResult<()> {
    ...
    event_loop.reregister(&self.sock, self.token, self.interest, evt::EDGE)
  }

  fn readable(&mut self, event_loop: &mut TestEventLoop) -> MioResult<()> {

    match self.sock.read(&mut self.buf) {
      Ok(NonBlock::WouldBlock) => panic!("We just got readable, but were unable to read from the socket?"),
...
impl EchoServer {
  fn accept(&mut self, event_loop: &mut TestEventLoop) -> MioResult<()> {
    ...
    event_loop.register_opt(&self.conns[tok].sock, tok, evt::READABLE, evt::EDGE)
...
impl EchoClient {
  ...
  fn readable(&mut self, event_loop: &mut TestEventLoop) -> MioResult<()> {
    ...
    let res = match self.sock.read(&mut self.buf) {
      Ok(NonBlock::WouldBlock) => panic!("We just got readable, but were unable to read from the socket?"),
        Ok(NonBlock::Ready(r)) => { debug!("CLIENT : We read {} bytes!", r); }
  ...
  fn writable(&mut self, event_loop: &mut TestEventLoop) -> MioResult<()> {
    ...
    match self.sock.write(&mut self.tx) {
      Ok(NonBlock::WouldBlock) => { debug!("client flushing buf; WOULDBLOCK");
                              self.interest.insert(evt::WRITABLE); }
            Ok(NonBlock::Ready(r)) => { debug!("CLIENT : we wrote {} bytes!", r);
                              self.interest.insert(evt::READABLE);
                              self.interest.remove(evt::WRITABLE); }
                  Err(e) => debug!("not implemented; client err={}", e)
     } 
  ...
  fn writable(&mut self, event_loop: &mut TestEventLoop, token: Token) {
    match token {
      ...
      CLIENT => self.client.writable(event_loop).unwrap(),
           i => self.server.conn_writable(event_loop, token).unwrap()
    };
  }
}
...
pub fn test_echo_server() {
  ...
  event_loop.register_opt(&srv, SERVER, evt::READABLE, evt::EDGE).unwrap();

  let sock = TcpSocket::v4().unwrap();

  // Connect to the server
  event_loop.register_opt(&sock, CLIENT, evt::WRITABLE, evt::EDGE ).unwrap();
