...
impl Echo {
    fn new(srv: TcpListener, client: TcpStream, msgs: Vec<&'static str>) -> Echo {
        Echo {
            ...
            client: EchoClient::new(client, STREAM, msgs),
        }
    }
}
...
fn test_echo_server() {
    ...
    poll.registry()
        .register(&sock, STREAM, Interests::WRITABLE)
        .unwrap();
        ...
        for event in &events {
            ...
            if event.is_readable() {
                match event.token() {
                    ...
                    STREAM => handler.client.readable(poll.registry()).unwrap(),
            ...
            if event.is_writable() {
                match event.token() {
                    ...
                    STREAM => handler.client.writable(poll.registry()).unwrap(),
